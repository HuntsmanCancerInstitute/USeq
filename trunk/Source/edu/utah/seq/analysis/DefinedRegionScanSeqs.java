...
public class DefinedRegionScanSeqs {
	...
	private boolean lookForAlternativeSplicing = true;
	...
	public DefinedRegionScanSeqs(File[] treatmentPointDirs, File[] controlPointDirs, File saveDirectory, File fullPathToR, File refSeqFile, boolean scoreIntrons, boolean verbose){
	...
	public void run(){
		//load gene models
		if (verbose) System.out.println("Loading region or gene table...");
		try {
			loadGeneModels();
		} catch (IOException e) {
		...
		if (treatmentSpliceFiles != null && controlSpliceFiles != null){
			if (verbose) System.out.println("Splitting splice junction bed files...");
		...
		if (verbose) System.out.print("Scanning regions by chromosome");
		...
		if (controlPointDirs != null){
			...
			//calculate on distribution of reads between the exons
			if (lookForAlternativeSplicing){
				if (verbose) System.out.println("Estimating read distribution, bonferroni corrected, chi-square p-values in R (very slow) ...");
			...
			if (totalNumberJunctionsScored != 0) {
				if (verbose) System.out.println("Scoring splice junctions...");
				scoreSpliceJunctions();
			}
		}
		//sort by score[0] pValue
		if (verbose) System.out.println("Printing results...");
	...
	public void loadGeneModels() throws IOException{
		...
		if (refSeqFile != null){
			...
			if (removeOverlappingRegions) {
				if (verbose) System.out.println("\tRemoving overlapping regions from gene models.\n");
		...
		//or from bed file
			...
			lookForAlternativeSplicing = false;
		...
		if (geneModels == null || geneModels.size() == 0) throw new IOException("Problem loading your USCS gene model table or bed file. No gene models?\n");
		//check ordering
	...
	private void sumTreatmentReadsForEnds(int numberTerminalBPs, float minimumNumberReadsEachSide){
		...
		for (int i=0; i< genes.length; i++){
			...
			if (strand.equals("+")){
				if (verbose) System.out.println(genes[i].getDisplayName()+"\t"+genes[i].getName()+"\t"+genes[i].coordinates()+"\t"+leftSum+"\t"+rightSum+"\t"+(Num.log2((1+rightSum)/(1+leftSum))));
			}
			else if (strand.equals("-")) if (verbose) System.out.println(genes[i].getDisplayName()+"\t"+genes[i].getName()+"\t"+genes[i].coordinates()+"\t"+rightSum+"\t"+leftSum+"\t"+(Num.log2((1+leftSum)/(1+rightSum))));
			else if (verbose) System.out.println(genes[i].getName()+"\tUnstranded!");
	...
	public HashMap<String, File> splitByChrom (File[] bedFiles){
		...
		try{
			...
			for (int i=0; i< bedFiles.length; i++){
				...
				if (verbose) System.out.println("\t"+bedFiles[i].getName());
				String[] tokens;
				String line;
				while ((line=in.readLine()) != null){
	...
	public void processArgs(String[] args){
		...
		if (refSeqFile == null && bedFile == null){
			Misc.printExit("\nPlease enter a region or gene table file to use in scoring.\n");
		}
		//info
		if (verbose){
