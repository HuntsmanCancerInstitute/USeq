...
public class ParseExonMetrics {
	...
	private void generateRBarplot(ArrayList<Integer> xvalues, ArrayList<Float> yvalues,String suffix, String title, String xaxis, String yaxis, File output) {
		...
		try {
			...
			//make command
			ProcessBuilder pb = new ProcessBuilder("/usr/local/bin/R","CMD","BATCH","--no-save","--no-restore",scriptFile.getCanonicalPath(),rOut.getCanonicalPath());
	...
	private void generateLatex(File latexFile) {
		...
		try {
			...
			bw.write("\\title{Sample Metrics: " + outputFile.getName() + "}\n");
			...
			bw.write("\\paragraph{}\n");
			bw.write("Alignment Statistics for standard chromosomes prior to recalibration, realignment and deduplication can be found in \\textbf{Table~\\ref{tab:2}}. \\emph{Data generated by Picard's "
