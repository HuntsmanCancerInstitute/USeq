...
public class Num {
	...
	public static double[][] normalizeCountMatrix (int[][] countMatrix){
		...
		for (int i=0; i<countMatrix.length; i++){
			//for each column
			for (int j=0; j< numColumns; j++){
				normalizedData[i][j] = scalars[j] * countMatrix[i][j];
			}
		}
	...
	public static float percentile(float[] sortedFloats, double percentile){
		//calculate index
		double index = ((percentile * sortedFloats.length)) - 0.5;
	...
	public static double matScore(float[] t, float[] c){
		...
		int trimNumberT = (int)Math.round(t.length * 0.1);
		...
		int trimNumberC = (int)Math.round(c.length * 0.1);
	...
	public static float[] medianNormalize (float[] f, float targetMedian){
		...
		System.arraycopy(f,0,sorted,0,num);
		for (int i=0; i<num; i++){
			Double scaled = new Double(sorted[i] * scalar);
	...
	public static float[][] medianNormalize (float[][] f, double targetMedian){
		...
		for (int i=0; i<num; i++){
			int num2= f[i].length;
			for (int j=0; j<num2; j++){
				Double scaled = new Double(f[i][j] * scalar);
	...
	public static float log2(float number){
		return (new Double (Math.log(number)/log2)).floatValue();
	}
	...
	public static double[] smoothScores(double[] scores, int[] positions, int windowSize){
		...
		int halfWindow = Math.round(windowSize/2);
		for (int i=0; i< num; i++){
	...
	public static double trimmedMean(double[] sortedFloat, double fraction){
		int trimNumber = (int)Math.round(sortedFloat.length * fraction);
	...
	public static float[] sam(float[] t, float[] c, double fudge){
		...
		double sT = Math.pow(standardDeviation(t, meanT), 2)/t.length;
		double sC = Math.pow(standardDeviation(c, meanC), 2)/c.length;
