		...
		try {
			...
			for (String geneName: geneVcf.keySet()){
				...
				for (VCFCount vc: hits){
					...
					//note position is zero based not the vcf 1 based
					out.print(vc.vcf.getChrPosRefAlt(true)); out.print("_"); out.print(snpId++);out.print("\t");
			...
			for (VCFCount vc: noGeneHit){
				//note position is zero based not the vcf 1 based
				String noName = vc.vcf.getChrPosRefAlt(true);
