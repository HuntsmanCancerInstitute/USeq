...
public class MethylationArrayCVScanner {
	...
	//for coeficient of variation analysis
	private boolean saveCVTracks = false;
	private File cvTrackDir = null;
	private File cvTreatmentDir = null;
	private File cvControlDir = null;
	private File cvRatioDir = null;
	...
	public void run(){
		...
		//calculate and save CV data?
		if (saveCVTracks) parseAndWriteCV();
	...
	private void parseAndWriteCV(){
		//make dir to hold results
		cvTrackDir = new File (saveDirectory, "CVTracks");
		cvTrackDir.mkdirs();
		cvTreatmentDir = new File (cvTrackDir, "TreatmentCV");
		cvControlDir = new File (cvTrackDir, "ControlCV");
		cvRatioDir = new File (cvTrackDir, "Log2RatioCV");
		cvTreatmentDir.mkdir();
		cvControlDir.mkdir();
		cvRatioDir.mkdir();
		
		//for each chromosome
		for (String chrom: chromMethylationArraySamplePairs.keySet()){
			ArrayList<float[]> treatment = new ArrayList<float[]>();
			ArrayList<float[]> control = new ArrayList<float[]>();
			
			MethylationArraySamplePair[] pairs = chromMethylationArraySamplePairs.get(chrom);
			//for each pair
			for (MethylationArraySamplePair p: pairs){
				float[] v = p.getTreatment();
				if (v != null) treatment.add(v);
				v = p.getControl();
				if (v != null) control.add(v);
			}
			//calc CVs
			float[] cvT = calcCV(treatment);
			float[] cvC = calcCV(control);
			
			saveCVGraphs(chrom, cvT, cvC);
		}
	}

	

	private void saveCVGraphs(String chrom, float[] cvT, float[] cvC) {
		//write graphs
		int[] positions = chromPositions.get(chrom);		
		writeCVGraph(cvTreatmentDir, chrom, positions, cvT, null);
		writeCVGraph(cvControlDir, chrom, positions, cvC, null);
		writeCVGraph(cvRatioDir, chrom, positions, cvT, cvC);
	}
	
	private void writeCVGraph(File dir, String chrom, int[] positions, float[] first, float[] second){
		PointData r = null;
		if (second == null) {
			r = new PointData();
			...
			for (int i=0; i< positions.length; i++){
				if (first[i] !=0 && second[i] !=0) {
					float ratio = Num.log2(first[i]/ second[i]);
					good.add(new Point(positions[i], ratio));
				}
			}
			//make PointData
			r = Point.extractPositionScores(good);
		}
		...
		//color orange
		map.put(BarParser.GRAPH_TYPE_COLOR_TAG, "#FF9900");
		//description
		if (second != null) map.put(BarParser.DESCRIPTION_TAG, "Log2Ratio of t/c probe coefficient of variation");
		else map.put(BarParser.DESCRIPTION_TAG, "Probe coefficient of variation");
		Info info = new Info(dir.getName(), versionedGenome, chrom, ".", 0, map);
		r.setInfo(info);
		r.writePointData(dir);
	}

	private float[] calcCV(ArrayList<float[]> al) {
		//make matrix
		float[][] matrix = new float[al.size()][];
		for (int i=0; i< matrix.length; i++) matrix[i] = al.get(i);
		
		float[] toCalc = new float[matrix.length];
		float[] cvs = new float[matrix[0].length];
		...
		for (int i=0; i< matrix[0].length; i++){
			...
			double mean = (double)total/(double)toCalc.length;
			cvs[i] = (float)(Num.standardDeviation(toCalc, mean)/mean);
		}
		return cvs;
	}
	...
	public HashMap<String, PointData> loadPointData(String name){
		...
		//check and or set positions, loads data!
		...
		for (int i = 0; i<args.length; i++){
			...
			if (mat.matches()){
				...
				try{
					...
					case 'e': runT = true; break;
					case 'v': saveCVTracks = true; break;
