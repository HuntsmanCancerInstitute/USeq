...
public class BarcodedPairedAlignmentChrParser extends Thread{
	...
	private MergeBarcodedPairedAlignments mbpa = null;
	...
	private int numberFailingMolecularBarcodeQC = 0;
	private int numberFailingFamilySize = 0;

	public BarcodedPairedAlignmentChrParser (String chromosome, MergeBarcodedPairedAlignments mbpa){
		//set params
		this.chromosome = chromosome;
		this.mbpa = mbpa;
		queryUnMapped = chromosome.equals("unmapped");
		crossCheckMateCoordinates = mbpa.isCrossCheckMateCoordinates();
		maximumProperPairDistanceForMerging = mbpa.getMaximumProperPairDistanceForMerging();
		secondPairReverseStrand = mbpa.isSecondPairReverseStrand();
		maximumAlignmentScore = mbpa.getMaximumAlignmentScore();
		minimumMappingQualityScore = mbpa.getMinimumMappingQualityScore();
		saveSams = mbpa.isSaveSams();
		removeDuplicates = mbpa.isRemoveDuplicates();
		minFamilySize = mbpa.getMinFamilySize();
	}
	...
	public void run(){
		try {
			...
			if (saveSams == false){
				...
				chromDataFile = new File(mbpa.getSaveDirectory(), chromosome+".ChromData");
			...
			Misc.printErrAndExit("Error parsing "+chromosome+" data from "+mbpa.getBamFile());
			e.printStackTrace();
		}
	}
	...
	public boolean checkSamAlignment(SamAlignment sa, String line) throws IOException{
		...
		//Passes minimum family size?
		if(sa.getFamilySizeTag() < minFamilySize){
			numberFailingFamilySize++;
			isGood = false;
		}
		//Passes fastq family collapsing QC
		if(!sa.getBarcodeQCTag()){
			numberFailingMolecularBarcodeQC++;
			isGood = false;
		}
	...
	public void parseAlignmentFile(){
		...
		try {
			...
			SamReader samReader = factory.open(mbpa.getBamFile());
			...
			//create gzipper?
			if (saveSams){
				String name = Misc.removeExtension(mbpa.getBamFile().getName());
				samOut = new Gzipper (new File (mbpa.getSaveDirectory(), chromosome+"_"+name+".sam.gz"));
				//add header to results file
				samOut.println(mbpa.getSamHeader());
			}
			...
			if (loadAlignments(mbpa.getNumberAlignmentsToLoad()) == false) processAlignmentHash();
	...
	public int getNumberFailingFamilySize() {
		return numberFailingFamilySize;
	}
	
	public int getNumberFailingMolecularBarcodeQC() {
		return numberFailingMolecularBarcodeQC;
	}
