...
/**
 * @author daniel.baker@aruplab.com
 * Forked from MergePairedAlignments but with extra functionality
 * to be barcode-aware.
 * Now works on coordinate and query name sorted files. Multi threaded.
 **/
public class MergeBarcodedPairedAlignments {
	...
	private int minFamilySize = Integer.MIN_VALUE;
	...
	private ArrayList<BarcodedPairedAlignmentChrParser> parsers;

	//constructors
	public MergeBarcodedPairedAlignments(String[] args){
		try {
	...
	public void doWork() throws IOException{
		...
		parsers = new ArrayList<BarcodedPairedAlignmentChrParser>();
		//add parser for unmapped reads, note this doesn't fetch all of them, need to scan rest of chromosomes
		parsers.add(new BarcodedPairedAlignmentChrParser("unmapped", this));
		//add chrom parsers
		for (int i=0; i< chromosomes.size(); i++) parsers.add(new BarcodedPairedAlignmentChrParser(chromosomes.get(i), this));
		//for (int i=23; i< 26; i++) parsers.add(new BarcodedPairedAlignmentChrParser(chromosomes.get(i), this));
	...
	private void runThreads() {
		while (true){
			try {
				//how many running
				int numRunning = 0;
				ArrayList<BarcodedPairedAlignmentChrParser> toStart = new ArrayList<BarcodedPairedAlignmentChrParser>();
				for (BarcodedPairedAlignmentChrParser p : parsers){
				...
				if (numToStart > 0){
					...
					for (int i=0; i< stop; i++){
						BarcodedPairedAlignmentChrParser p = toStart.get(i);
						p.start();
	...
	private void collectStats() {
		try {			
			...
			for (int i=0; i< parsers.size(); i++){
				BarcodedPairedAlignmentChrParser parser = parsers.get(i);
		...
		for (int i = 0; i<args.length; i++){
			...
			if (mat.matches()){
				...
				try{
					...
					case 'f': minFamilySize = Integer.parseInt(args[++i]); break;
	...
	public static void printDocs(){
		System.out.println("\n" +
				"**************************************************************************************\n" +
				"**                            Merge Barcoded Paired Alignments: May 2015                     **\n" +
				"**************************************************************************************\n" +
	...
	public int getMinFamilySize(){
		return minFamilySize;
	}
}
