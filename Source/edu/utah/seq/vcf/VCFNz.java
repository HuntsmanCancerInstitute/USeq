...
import edu.utah.seq.parsers.mpileup.MpileupTabixLoaderNz;
...
public class VCFNz {
	...
	private double maximumZScore = 0;
	private int bpPad = 0;
	...
	private MpileupTabixLoaderNz[] loaders = null;
	...
	//constructor
	public VCFNz(String[] args){
		long startTime = System.currentTimeMillis();
		try {	
			processArgs(args);
			
			//load seq stats
			seqMeanStd = (HashMap<String, double[]>)IO.fetchObject(seqStats);

			//make Loaders
			loaders = new MpileupTabixLoaderNz[numberThreads];
			for (int i=0; i< loaders.length; i++) loaders[i] = new MpileupTabixLoaderNz(mpileup, this);

			//for each vcf file
			IO.pl("\nParsing vcf files...");
			IO.pl("Name\tRecords\tSaved\tNotScored\tFailingNZScore");
			...
			for (MpileupTabixLoaderNz l: loaders) executor.execute(l);
			executor.shutdown();
			while (!executor.isTerminated()) {}

			//check loaders 
			for (MpileupTabixLoaderNz l: loaders) {
			...
			writeOutModifiedVcf (finalVcfs, new File(saveDir, name+".nz.vcf.gz"));

			IO.pl("\t"+numRecords+"\t"+numSaved+"\t"+numNotScored+"\t"+numFailingZscore);
			...
			//shut down loaders
			for (MpileupTabixLoaderNz m : loaders) m.getTabixReader().close();
	...
	public synchronized void update(int numNotScored, int numFailingZscore) {
	...
	public void processArgs(String[] args){
		...
		for (int i = 0; i<args.length; i++){
			...
			if (mat.matches()){
				...
				try{
					...
					case 'r': saveDir = new File(args[++i]); break;	
					case 'm': mpileup = new File(args[++i]); break;
					case 's': seqStats = new File(args[++i]); break;
					case 'f': indexedFasta = new File(args[++i]); break;
					case 'z': maximumZScore = Double.parseDouble(args[++i]); break;
		...
		//check vcf file
		if (seqStats == null || seqStats.canRead() == false) Misc.printExit("\nError: cannot find your xxx.mstats.obj) file! See MpileupStats app.\n");
				
		//check index fasta
		if (indexedFasta == null || indexedFasta.exists() == false)  Misc.printErrAndExit("\nError: cannot find indexed fasta file? -> "+ indexedFasta);
		
		//save dir?
		if (saveDir == null) saveDir = vcfFile.getParentFile();
		else saveDir.mkdirs();
	...
	public void printSettings(){
		IO.pl("Settings:");
		IO.pl(" -v Vcf file "+ vcfFile);
		IO.pl(" -m Mpileup  "+ IO.getCanonicalPath(mpileup));
		IO.pl(" -s Save dir    "+ IO.getCanonicalPath(saveDir));
		IO.pl(" -x "+ minBaseQuality+"\tMin mpileup sample base quality");
		IO.pl(" -x "+ maximumZScore+"\tMax N z-score");
		IO.pl(" -x "+ bpPad+"\tBP padding +/- to vcf record for mpileup scanning");
		IO.pl(" -x "+ numberThreads+"\tCPUs");
		IO.pl(" -x "+ removeNonZScoredRecords+ "\tExclude vcf records that could not be z-scored");
	...
	public double getMaximumZScore() {
		return maximumZScore;
	}
	...
	public HashMap<String, double[]> getSeqMeanStd() {
		return seqMeanStd;
	}

	public File getIndexedFasta() {
		return indexedFasta;
	}
}
