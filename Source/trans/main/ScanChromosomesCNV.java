 ...
 */
public class ScanChromosomesCNV {
	...
	public ScanChromosomesCNV(String[] args){
		System.out.println("\n\nONLY FOR CNV ANALYSIS! Differences, not ratios/ rel diff.\n\n");		
		//process args
		processArgs(args);
		...
		for (int i=0; i<chromosomeOligoPositions.length; i++){
			...
			for (int j=start; j<stop; j++){
				...
				double wrs = wt.test(treatmentIntensities, controlIntensities, startStop[0], startStop[1]);
				...
				if (convertScoresToQValues || printPointBars){
					...
					if (printPointBars){
						...
						chrScoreVal[barIndex] = new Double (score).floatValue();
	...
	public double scoreWindow(int testWindowIndex){
		...
		score = Num.meanIgnoreNaNInfinity(windowRelDiffs);
	...
	public void printOligoRatios(){
		...
		int[] posChr;
		float[] valChr;
		ArrayList<Integer> posAL = new ArrayList<Integer>(numberOligoMeasurements);
		ArrayList<Float> valAL = new ArrayList<Float>(numberOligoMeasurements);

		double[] meanC = null; 

		meanC = Num.averageFloatArraysFlipped(controlIntensities);
		//for every measurement, calculate difference
		for (int j=0; j<numberOligoMeasurements; j++){
			double ratio = meanT[j] - meanC[j];
			//skip if Infinite or NaN
			if (Double.isInfinite(ratio) || Double.isNaN(ratio)) continue;
			posAL.add(new Integer(positions[j]+halfOligoLength));
			valAL.add(new Float(ratio));	
		}

		meanC = null;
		meanT = null;
		posChr = Num.arrayListOfIntegerToInts(posAL);
		valChr = Num.arrayListOfFloatToArray(valAL);
		posAL = null;
		valAL = null;
		BarParser bp = new BarParser();
		bp.setZipCompress(true);
	...
	public void makeChromosomeWideScores(){
		...
		//make chromosome wide oligo relative differences
			...
			if (pmOnly) {
				System.out.println("\tCalculating singleScore differences.");
				//singleScores = Num.relativeDifferences(treatmentIntensities, controlIntensities);
				singleScores = Num.difference(treatmentIntensities, controlIntensities);
				System.out.println("Making differences");
			}
	...
	public void buildExtensionSetThreshold(){
		...
		if (usePseudoMedian) {
			...
			extension = "Diff";
			System.out.print("a mean on average replica oligo ratios.");
		}
