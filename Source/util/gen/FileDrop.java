...
{
	private transient javax.swing.border.Border normalBorder = BorderFactory.createLineBorder(Color.black);
	...
	private static java.awt.Color defaultBorderColor = Color.BLACK;
	...
	{   
		...
		{   // Make a drop listener
			...
			{   // Reject the drag event
				evt.rejectDrag();
				log( out, "FileDrop: event rejected 1." );
			}   // stop else: drag not ok
			}   // stop dragEnter
			...
			{   // Get whatever was dropped
				...
				}   // stop else: not a file list
			}   // stop try
			catch ( java.io.IOException io) 
			{   log( out, "FileDrop: IOException - abort:" );
			io.printStackTrace( out );
			evt.rejectDrop();
			}   // stop catch IOException
			catch (java.awt.datatransfer.UnsupportedFlavorException ufe) 
			{   log( out, "FileDrop: UnsupportedFlavorException - abort:" );
			ufe.printStackTrace( out );
			evt.rejectDrop();
			}   // stop catch: UnsupportedFlavorException
			finally
			{
				...
				log( out, "FileDrop: normal border restored 1." );
				}   // stop if: JComponent
			}   // stop finally
			}   // stop drop
			...
			{   javax.swing.JComponent jc = (javax.swing.JComponent) c;
			jc.setBorder( normalBorder );
			log( out, "FileDrop: normal border restored 2." );
			}   // stop if: JComponent
			}   // stop dragExit
			...
			{   evt.rejectDrag();
			log( out, "FileDrop: event rejected 2." );
			}   // stop else: drag not ok
			}   // stop dropActionChanged
			}; // stop DropTargetListener

			// Make the component (and possibly children) drop targets
			makeDropTarget( out, c, recursive );
		}   // stop if: supports dnd
		else
		{   log( out, "FileDrop: Drag and drop is not supported with this JVM" );
		}   // stop else: does not support DnD
	}   // stop constructor
	...
	private static File[] createFileArray(BufferedReader bReader, PrintStream out)
	{
		try { 
			...
			while ((line = bReader.readLine()) != null) {
				try {
					// kde seems to append a 0 char to the stop of the reader
	...
	private void makeDropTarget( final java.io.PrintStream out, final java.awt.Component c, boolean recursive )
	{
		...
		}   // stop else: parent not null
		}   // stop hierarchyChanged
		}); // stop hierarchy listener
		if( c.getParent() != null )
			new java.awt.dnd.DropTarget(c, dropListener);
		...
		}   // stop if: recursively set components as listener
	}   // stop dropListener



	/** Determine if the dragged data is a file list. */
	public boolean isDragOk( final java.io.PrintStream out, final java.awt.dnd.DropTargetDragEvent evt )
	{   boolean ok = false;
	...
	{   
		...
		// END 2007-09-12 Nathan Blomquist -- Linux (KDE/Gnome) support added.
		i++;
	}   // stop while: through flavors

	// If logging is enabled, show data flavors
	if( out != null )
	{   if( flavors.length == 0 )
		log( out, "FileDrop: no data flavors 1." );
	for( i = 0; i < flavors.length; i++ )
		log( out, flavors[i].toString() );
	}   // stop if: logging enabled
	...
	public static class TransferableObject implements java.awt.datatransfer.Transferable
	{
		...
		}   // stop inner interface Fetcher



	}   // stop class TransferableObject
